...
- **Framework-contract fields** (`Model`, `Portal`, `Fallback`, `CustomPortals`) — set only by concrete subclass addons, no module owner. Declare them on a `---@class` block at the top of the base's `shared.lua`. The two base classes are declared `---@class gmod_door_exterior : Entity` / `… : Entity` so engine methods resolve, and their pairing cross-refs (`interior` on the exterior, `exterior` on the interior, plus `door` / `doori` on `Player` in `sh_players.lua`) are declared `---@field … ?` — without those declarations the inferred field types don't carry `IsValid` narrowing (see the diagnostics note below).
...
The `.luarc.json` `diagnostics.disable` block is down to a single rule, `need-check-nil`. `param-type-mismatch` was re-enabled via the enum-constant re-typing in `glua_overrides.lua` (above). `unchecked-nil-access` was re-enabled by giving the framework cross-references real types — the `: Entity` base and the `interior` / `exterior` / `door` / `doori` field declarations described in the **Framework-contract fields** bullet. `IsValid()` *does* narrow nilable values for glua_ls (a plain `if IsValid(x) then x:m() end` is clean); the old false positives came from those fields being *inferred* from scattered `x = nil` / `x = ents.Create(...)` assignments across files, and inferred cross-file field types don't carry the narrowing. With the fields declared, the only genuine hit left was the interior's portal setup dereferencing `self.exterior` with no guard (it assumes a live exterior) — fixed with an explicit `IsValid` early-return.

`need-check-nil` stays disabled. Its hints land on hook-callback `self`: inside `ENT:AddHook(name, id, function(self) … end)` the analyzer infers `self` as an anonymous shape from its field uses, not as the entity class, so engine methods like `self:LocalToWorld` read as possibly-nil. Typing the callback `self` (annotating `AddHook`'s `func` param as `fun(self: gmod_door_interior, ...)`) fixes those, but cascades the rule onto the framework's duck-typed config tables — `self.CustomPortals` then resolves and every `v.entry` / `v.exit` / `v.pos` becomes a possibly-nil hint (13 → 37). Net more noise than signal on intentionally-dynamic data. The rule is hint-severity so it never fails CI; revisit if we ever declare the `Portal` / `CustomPortals` entry shapes.
