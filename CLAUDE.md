...
The `.luarc.json` carries a **small `diagnostics.disable` block** — now just the two nil rules (`need-check-nil`, `unchecked-nil-access`). `param-type-mismatch` was re-enabled: its only false positives were the four enum-constant call sites, fixed centrally by the constant re-typing in `glua_overrides.lua` (above), so the rule now guards real type mismatches at zero source cost.

The two nil rules stay disabled because they cannot see `IsValid()` as a nil-narrowing guard — no LuaLS-family analyzer supports a type predicate on a boolean-returning function, and `IsValid` is *the* idiomatic GMod nil+validity check. They fire on the nilable framework cross-refs (`self.interior` / `self.exterior` / `ply.door` / `ply.doori`) even inside `if IsValid(x) then …` blocks, plus on pairing invariants the analyzer can't know (a live interior always has an exterior) and on hook-callback `self` whose Entity methods don't resolve. Every current hit is a false positive on correctly-guarded code, so re-enabling adds editor noise (16 warnings + 19 hints) with no real-bug catch. Revisit if glua_ls gains `IsValid`-aware narrowing.
